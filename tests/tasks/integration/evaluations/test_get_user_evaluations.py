...
from app.tasks.routers.api.evaluations import EVALUATION_GET_USER_EVALUATIONS_ROUTE_NAME
...
class TestGetUserEvaluations(GetUrlMixin):
    url_name = EVALUATION_GET_USER_EVALUATIONS_ROUTE_NAME

    @property
    def _some_deadline(self) -> datetime:
        return datetime.now(UTC) + timedelta(minutes=10)

    async def test_get_user_evaluations(
        self,
        async_client,
        create_user,
        create_team,
        ...
        assert (
            json.loads(EvaluationRead.model_validate(evaluation3).model_dump_json())
            not in data
        )

    async def test_get_user_evaluations_unauthenticated(self, async_client):
        response = await async_client.get(
            self.get_url(),
        )
    ...
    async def test_get_user_evaluations_inactive(
    ...
    async def test_get_user_evaluations_for_period(
        self,
        async_client,
        session,
        create_user,
    ...
    async def test_get_user_evaluations_for_period_start_later_than_end(
        self,
        async_client,
        create_user,
        get_authorization_header,
