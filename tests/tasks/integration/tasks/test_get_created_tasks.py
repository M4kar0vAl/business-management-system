...
from app.tasks.routers.api.tasks import GET_CREATED_TASKS_ROUTE_NAME
...
class TestGetCreatedTasks(GetUrlMixin):
    url_name = GET_CREATED_TASKS_ROUTE_NAME

    @property
    def _some_deadline(self) -> datetime:
        return datetime.now(UTC) + timedelta(minutes=10)

    async def test_get_created_tasks(
        self,
        async_client,
        create_user,
        create_team,
    ...
    async def test_get_created_tasks_unauthenticated(
        self, async_client, create_user, create_team, create_task
    ):
        user, _ = await create_user(
            UserCreate(email="user@example.com", password="Pass!234", role=Role.MANAGER)
        )
        team = await create_team(TeamCreate(name="team"), members=[user])
        await create_task(
            TaskCreate(
    ...
    async def test_get_created_tasks_inactive(
        self,
        async_client,
        create_user,
        create_team,
    ...
    async def test_get_created_tasks_non_manager(
        self,
        async_client,
        create_user,
        create_team,
    ...
    async def test_get_created_tasks_user_not_a_member_of_team(
        self,
        async_client,
        session,
        create_user,
