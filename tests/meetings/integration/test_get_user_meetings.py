...
from app.meetings.routers import GET_USER_MEETINGS_ROUTE_NAME
from app.meetings.schemas import MeetingCreate, MeetingRead
from tests.mixins import GetUrlMixin


class TestGetUserMeetings(GetUrlMixin):
    url_name = GET_USER_MEETINGS_ROUTE_NAME
    ...
    async def test_get_user_meetings(
        ...
        today_meeting = await create_meeting(
            self._today_meeting_create, user, participants=[user]
        )
        tomorrow_meeting = await create_meeting(
            self._tomorrow_meeting_create, user, participants=[user]
        )
        day_after_tomorrow_meeting = await create_meeting(
            self._day_after_tomorrow_meeting_create,
            another_user,
            participants=[another_user],
        )
    ...
    async def test_get_user_meetings_unauthenticated(self, async_client):
        response = await async_client.get(self.get_url())

        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    async def test_get_user_meetings_inactive(
    ...
    async def test_get_user_meetings_for_period(
        self, async_client, create_user, create_meeting, get_authorization_header
    ):
        user, token = await create_user(
            UserCreate(email="user@example.com", password="Pass!234"), authenticate=True
        ...
        tomorrow_meeting = await create_meeting(
            self._tomorrow_meeting_create, another_user
        )
        day_after_tomorrow_meeting = await create_meeting(
            self._day_after_tomorrow_meeting_create, user, participants=[user]
        )
    ...
    async def test_get_user_meetings_for_period_start_in_future(
        self, async_client, create_user, get_authorization_header
    ):
        _, token = await create_user(
            UserCreate(email="user@example.com", password="Pass!234"), authenticate=True
